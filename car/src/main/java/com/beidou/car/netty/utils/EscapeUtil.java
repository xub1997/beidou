package com.beidou.car.netty.utils;
...
public class EscapeUtil {
    private static Logger logger= LoggerFactory.getLogger(EscapeUtil.class);
    /**
     * 接收消息时转义<br>
     * <p>
     * <pre>
    ...
    public static int generateMsgBodyProps(int msgLen, int enctyptionType, boolean isSubPackage, int reversed_14_15) {
        ...
        if (msgLen >= 1024)
            logger.warn("The max identity of msgLen is 1023, but {} .", msgLen);
     ...
     * </pre>
     *
     * @param phone           终端手机号 bcd[6]
     * @param msgType         消息ID word(16)
     * @param body            消息体
     * @param msgBodyProps    消息体属性 word(16)
     * @param flowId          消息流水号 word(16),按发送顺序从 0 开始循环累加
     * @return                转义后的字节数组
     * @throws Exception
     */
    public static byte[] generateMsgHeader(String phone, int msgType, byte[] body, int msgBodyProps, int flowId)
        ...
        try {
            baos = new ByteArrayOutputStream();
            // 1. 消息ID word(16)
            baos.write(BiteUtil.integerTo2Bytes(msgType));
            // 2. 消息体属性 word(16)
            baos.write(BiteUtil.integerTo2Bytes(msgBodyProps));
            // 3. 终端手机号 bcd[6]
            baos.write(BCDUtil.string2Bcd(phone));
            // 4. 消息流水号 word(16),按发送顺序从 0 开始循环累加
            baos.write(BiteUtil.integerTo2Bytes(flowId));
            // 消息包封装项 此处不予考虑
            return baos.toByteArray();
        } finally {
            if (baos != null) {

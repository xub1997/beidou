package com.beidou.position.websocket.handler;

import java.time.LocalDateTime;

import entity.GlobalChannelMap;
...
public class WebSocketHandler extends SimpleChannelInboundHandler<Object> {
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
    	//记录管道
    	GlobalChannelMap.group.add(ctx.channel());
    ...
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
    	GlobalChannelMap.group.remove(ctx.channel());
        System.out.println(ctx.channel()+"客户端与服务端连接关闭...");
    }
	...
	public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
		if (evt instanceof IdleStateEvent) {
			...
			if (event.state() == IdleState.READER_IDLE) {
				loss_connect_time++;
				System.out.println("10 秒没有接收到客户端的信息了，失去連接次數："+loss_connect_time);
    ...
    private void handWebsocketFrame(ChannelHandlerContext ctx, WebSocketFrame frame) {
        ...
        if (frame instanceof TextWebSocketFrame) {
            ...
            loss_connect_time=0;
            System.out.println("收到客户端" + GlobalChannelMap.parseChannelRemoteAddr(ctx.channel()) + "的消息==》"+requestMsg);
            String[] array= requestMsg.split(",");
            // 将通道加入通道管理器。array[0]是发送者，1是接收者，2是具体消息内容
            GlobalChannelMap.add(array[0],ctx.channel());
            if (array.length == 3) {
                // 表示是用户之间端到端的私密通信
                ...
                MsgManager.sendP2PMessage(senderId, receiverId, message);
            } else if (array.length == 2) {
                //如果没有指定接收者表示群发array.length() = 2
                System.out.println("用户" + array[0] + "群发了一条消息：" + requestMsg);
                GlobalChannelMap.group.writeAndFlush(new TextWebSocketFrame("用户" + array[0] + "群发了一条消息：" + requestMsg));
            } else {
